...
Matrix::Matrix(const int row, const int column){
	...
	rowsValue = row;
	columnsValue = column;
    for (unsigned int i = 0; i < rowsValue; i++) {
		matrix.push_back(std::vector<double>{});
        for (unsigned int j = 0; j < columnsValue; j++) {
...
double Matrix::get(const unsigned int row, const unsigned int col) const
{
...
Matrix Matrix::transpose(const Matrix& matrix)
{
    ...
    for (unsigned int i = 0; i < matrix.rowsValue; i++) {
        for (unsigned int j = 0; j < matrix.columnsValue; j++) {
...
Matrix& Matrix::operator=(const Matrix& m)
{
    ...
    for (unsigned int i = 0; i < m.rowsValue; i++) {
		matrix.push_back(std::vector<double>{});
        for (unsigned int j = 0; j < m.columnsValue; j++) {
...
Matrix& Matrix::operator*=(const Matrix& m)
{
    ...
    for (unsigned int i = 0; i < temp.rowsValue; i++) {
        for (unsigned int j = 0; j < temp.columnsValue; j++) {
            for (unsigned int k = 0; k < columnsValue; k++) {
...
void Matrix::addCol(std::vector<double> col)
{
	if (rowsValue < col.size()) {
        for (unsigned int i = 0; i < col.size() - rowsValue; i++) {
			matrix.push_back(std::vector<double>{});
		}
		rowsValue = col.size();
	}
	else if (rowsValue > col.size()) {
        for (unsigned int i = 0; i < rowsValue - col.size(); i++) {
			col.push_back(0.0);
		}
	}
...
void Matrix::addRow(const Matrix& from, const int rowIndex)
{
	if (columnsValue < from.columnsValue) {
        for (unsigned int i = 0; i < from.columnsValue - columnsValue; i++) {
			matrix[i].push_back(0.0);
		}
	...
	matrix.push_back(std::vector<double>{});
	rowsValue++;
    for (unsigned int i = 0; i < from.columnsValue; i++) {
...
void Matrix::addCol(const Matrix& from, const int colIndex)
{
	if (rowsValue < from.rowsValue) {
        for (unsigned int i = 0; i < from.rowsValue - rowsValue; i++) {
			matrix.push_back(std::vector<double>{});
		}
		rowsValue = from.rowsValue;
	}

    for (unsigned int i = 0; i < from.rowsValue; i++) {
		matrix[i].push_back(from.matrix[i][colIndex]);
	}
    for (unsigned int i = 0; i < rowsValue - from.rowsValue; i++) {
...
Matrix Matrix::getRow(const int row) const
{
    ...
    for (unsigned int i = 0; i < columnsValue; i++) {
...
{
    for (unsigned int i = zeroRow+1; i < rowsValue; i++) {
...
std::ostream& operator<<(std::ostream& os, const Matrix& m)
{
    for (unsigned int i = 0; i < m.rowsValue; i++) {
		os << m.matrix[i][0];
        for (unsigned int j = 1; j < m.columnsValue; j++) {
			os << " " << m.matrix[i][j];
		}
		os << std::endl;
	}
...
Matrix fullEchelonForm(const Matrix& extendedMatrix)
{
    ...
    for (unsigned int i = 0; i < temp.rowsValue - 1; i++) {
        ...
        for (unsigned int j = i + 1; j < temp.rowsValue; j++) {
