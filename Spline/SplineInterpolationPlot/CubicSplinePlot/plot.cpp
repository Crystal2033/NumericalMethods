...
void Plot::initPoints()
{
    ...
    const int valueOfIterations = (int)(abs(gridFunction[gridFunction.size()-1].x() - gridFunction[0].x()))/step + 1;
    std::vector<QPointF> baseGraphic;
    double startXValue = gridFunction[0].x();
    for(int i = 0; i < valueOfIterations; i++){
        baseGraphic.push_back(QPointF(startXValue + step * i,
                                getBaseFuncRes(startXValue + step * i)));

    }
    penAndGraphics.push_back(std::make_pair(QPen(Qt::white, 1), baseGraphic));

    std::vector<QPointF> splinedGraphic;

    for(int i = 0; i < valueOfIterations; i++){
        splinedGraphic.push_back(QPointF(startXValue + step * i,
                                getSplinedFuncRes(startXValue + step * i)));
    }
    penAndGraphics.push_back(std::make_pair(QPen(Qt::green, 1), splinedGraphic));
    update();
}

double Plot::getBaseFuncRes(const double x)
{
    return x * cos(x); //was Nc * x + sin(x), also can see x*sin(x)
}

double Plot::getSplinedFuncRes(const double x) const
{
    return spline->getSplineValue(x);
}


void Plot::create(const int delta) // creating plot
{
    std::vector<std::pair<double, double>> gridFunc;
    for(int i = -FUNCTION_X_RANGE/2; i <= FUNCTION_X_RANGE/2; i+=delta){
        gridFunc.push_back(std::make_pair(i, getBaseFuncRes(i)));
    }

    setGridPointsOnPlot(gridFunc);
    spline = new CubicSpline(gridFunc);
    spline->check();

    this->xMax = getMaxArgFromGrid();
...
void Plot::updateObserver(int deltaValue)
{
    create(deltaValue);
}
