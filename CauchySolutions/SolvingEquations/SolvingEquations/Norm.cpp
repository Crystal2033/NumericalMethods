...
double NormaVectorMaxVal::countNorm(const Matrix& matrix) const
{
	...
	if (matrix.rowsValue == 1) {
        for (unsigned i = 0; i < matrix.columnsValue; i++) {
            maxResult = std::max(maxResult, abs(matrix.matrix[0][i]));
		}
	}
	else if (matrix.columnsValue == 1) {
        for (unsigned i = 0; i < matrix.rowsValue; i++) {
            maxResult = std::max(maxResult, abs(matrix.matrix[i][0]));
		}
	}
	return maxResult;
}
...
double NormaMatrixMaxSumInRows::countNorm(const Matrix& matrix) const
{
    ...
    for (unsigned i = 0; i < matrix.rowsValue; i++) {
        maxResult = std::max(maxResult, currResult);
		currResult = 0.0;
...
double NormaMatrixMaxSumInCols::countNorm(const Matrix& matrix) const
{
    ...
    for (unsigned i = 0; i < matrix.columnsValue; i++) {
        maxResult = std::max(maxResult, currResult);
		currResult = 0.0;
        for (unsigned j = 0; j < matrix.rowsValue; j++) {
